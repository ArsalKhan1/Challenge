...
def preprocess_data(df):
    # Extract the required columns for BERT input
    ...
    # Extract the labels (multi-label classification for toxic content)
...
def save_model(model, path='./toxic-classifier-preprocessed-bert'):
    model.save_pretrained(path)

# Main function to run the whole process
def main():
    # Set device to MPS if available, otherwise CPU
    device = torch.device("mps") if torch.backends.mps.is_available() else print("mps not available")
