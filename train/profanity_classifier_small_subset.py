...
def load_data(sample_size=None):
    ...
    train_bias = pd.read_csv('../jigsaw-multilingual-toxic-comment-classification/new/jigsaw-toxic-comment-train-processed-seqlen128.csv')
    train_bias = pd.read_csv('../jigsaw-multilingual-toxic-comment-classification/new/jigsaw-unintended-bias-train-processed-seqlen128.csv')
    validation_data = pd.read_csv('../jigsaw-multilingual-toxic-comment-classification/validation-processed-seqlen128.csv')
    
    # If sample_size is provided, take a small random sample from each dataset
    if sample_size:
        train_toxic = train_toxic.sample(sample_size)
        train_bias = train_bias.sample(sample_size)
        validation_data = validation_data.sample(sample_size)
...
def main():
    ...
    device = torch.device("mps") if torch.backends.mps.is_available() else print("Sorry")
    
    # Load data (use a smaller sample size for testing, e.g., 100 samples)
    print("Loading data...")
    sample_size = 100  # Adjust sample size to a small number for testing
    train_toxic, train_bias, validation_data = load_data(sample_size=sample_size)
