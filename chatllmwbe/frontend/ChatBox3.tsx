...
export function ChatBox() {
  ...
  const submitUserInput = async () => {
    ...
    await getLLMResponse(
      ...
      (chunk) => {
        chatStore.appendToBotResponse(chunk); // Update each response chunk to the latest assistant message
      }
    );
  };
          ...
          {chatStore.curConversation()?.messages.map((item, i) => (
            <div key={i} className={`chat ${item.type === 'user' ? 'chat-end' : 'chat-start'}`}>
              <div className="chat-bubble">
                {item.type === 'assistant' ? (
                  <Markdown message={{ content: item.content }} fontSize={14} defaultShow={true} />
                ) : (
