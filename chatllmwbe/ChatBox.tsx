...
import { fetchLLMResponse } from '@/utils/api'; // Import the fetchLLMResponse function
...
export function ChatBox() {
  ...
  const submitUserInput = async () => {
    ...
    try {
      // Display the loading animation or initial message
      chatStore.appendMessage({ type: 'assistant', content: '', isLoading: true });
      const model = "gpt-4"; // Set the default model or let the user choose

      await fetchLLMResponse(userInput, model, true).then(async (streamedResponse) => {
        // Remove loading state and append final message to chat store
        chatStore.updateLastMessage("assistant", streamedResponse);
      });
    } catch (error) {
      console.error("Error fetching response:", error);
      chatStore.appendMessage({ type: 'assistant', content: 'Error: Unable to fetch response.' });
    }
  };
